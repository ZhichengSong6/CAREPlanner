...
LOCAL_WORKERS="${LOCAL_WORKERS:-4}";mkdir -p "$BASE/logs" "$BASE/submission_guards"
...
fi;RUN_SCRIPT="$HERE/run_global_2node.sh";export REPO VIS_PYTHON OUT BASE LOCAL_WORKERS RUN_SCRIPT
RAW="$(sbatch --parsable --partition=GPU --nodes=2 --ntasks=2 --ntasks-per-node=1 --cpus-per-task=8 --gres=gpu:4 --nodelist="${NODELIST:-3090node1,3090node3}" --time="${TIME_LIMIT:-04:00:00}" --output="$BASE/logs/global_%j.out" --export=ALL "$HERE/worker.sbatch")"
