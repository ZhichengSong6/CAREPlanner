...
if [[ -f "$MARKER" ]];then
 ...
 [[ -z "$ACTIVE" ]] || { echo "[STOP] existing job $OLD_JOB is active; use watch.sh v3 status";exit 2; }
 [[ "${RESUME:-false}" == true ]] || { echo "[STOP] stage was already submitted as $OLD_JOB; set RESUME=true only after confirming it ended";exit 2; }
fi;RUN_SCRIPT="$HERE/run_v3_2node.sh";export REPO VIS_PYTHON OUT BASE LOCAL_WORKERS RUN_SCRIPT
RAW="$(sbatch --parsable --partition=GPU --nodes=2 --ntasks=2 --ntasks-per-node=1 --cpus-per-task=16 --gres=gpu:1 --nodelist="${NODELIST:-3090node1,3090node3}" --time="${TIME_LIMIT:-16:00:00}" --output="$BASE/logs/v3_%j.out" --export=ALL "$HERE/worker.sbatch")"
JOB="${RAW%%;*}";printf '%s\n' "$JOB" >"$MARKER";JSTATE="$BASE/v3_job_${JOB}.env";printf 'export JOB_ID=%q\nexport OUT=%q\nexport LOG=%q\nexport STAGE=%q\n' "$JOB" "$OUT" "$BASE/logs/v3_${JOB}.out" v3 >"$JSTATE";cp "$JSTATE" "$BASE/latest_v3.env"
