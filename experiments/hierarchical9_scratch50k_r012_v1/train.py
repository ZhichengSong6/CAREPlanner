...
# Load frozen upstream scratch helpers without letting their absolute imports
# resolve this experiment's local model.py.
...
try:
    ...
    if _old_objective is None:
        sys.modules.pop("objective", None)
    else:
        sys.modules["objective"] = _old_objective
...
def _connected_early_zero(model) -> torch.Tensor:
    zero = None
    for p in model.early_parameters():
        term = p.sum() * 0.0
        zero = term if zero is None else zero + term
    if zero is None:
        raise RuntimeError("R2 model has no early parameters")
    return zero
...
def selected_sensor_local_objective(model, batch, counts9, weights, sensor_id: int) -> torch.Tensor:
    ...
    Denominator is global. Summing gradients across ranks gives L_s. Selecting
    one sensor per step provides the round-robin shared update while avoiding
    simultaneous sensor-sensor gradients on the shared early trunk.
    """
    ...
    if denom <= 0 or not valid.any():
        # A rank-local microbatch can legitimately contain no rows for the selected
        # sensor. Keep autograd connected to every early parameter and contribute 0.
        return _connected_early_zero(model)

    q = inputs[:, 3:10].detach().float().clone().requires_grad_(True)
    x = inputs[:, :3].detach().float()
    y = model.forward_sensor(
        torch.cat((x, q), dim=1), sensor_id, freeze_sensor_early=False
    ).float()
    ...
    cosine = F.cosine_similarity(
        grad_q[valid], target_grad[valid, sensor_id].float(), dim=-1, eps=1e-6
    )
    ...
    if weights.tension > 0:
        hvec = torch.autograd.grad(
            grad_q.sum(), q, create_graph=True, retain_graph=True
        )[0].float()
...
def run_r2_batch(
    ...
    for attempt in range(args.max_amp_retries + 1):
        ...
        for i, batch in enumerate(batches):
            ...
            # Main original objective: private branches receive their full losses.
            # Sensor paths use detached early parameters, not detached activations,
            # therefore q-gradient/eikonal/tension remain exact.
            with ddp.module.sensor_early_parameter_frozen():
                ...
                base.synchronized_check(
                    ...
                    "Non-finite R2 main objective",
                    device,
                )
                scaler.scale(loss).backward()

            # Selected sensor contributes its original head objective to shared early.
            with torch.enable_grad(), torch.autocast(device_type=device.type, enabled=False):
                aux = selected_sensor_local_objective(
                    ddp.module, batch, counts, weights, selected
                )
        ...
        # Each rank computed local numerator / global denominator. SUM gives the
        # desired global selected-sensor gradient; DDP is bypassed only for aux.
        for g in early_aux:
        ...
        for p, g in zip(early, early_aux):
            if p.grad is None:
                p.grad = g.to(dtype=p.dtype)
            else:
                p.grad.add_(g.to(dtype=p.grad.dtype))
    ...
    result.update(
        ...
        selected_shared_sensor=selected,
    )
    return result


def source_fingerprints() -> dict[str, str]:
    paths = [HERE / n for n in ("model.py", "train.py")] + [
        SCRATCH / "objective.py",
        SCRATCH / "train.py",
    ]
...
def save_checkpoint(
    ...
    if dist.get_rank() == 0:
        payload = {
            "format": FORMAT,
            "arm": args.arm,
            "completed": path.name == "final.pt",
            "model_state": ddp.module.state_dict(),
            ...
            "routing": (
                "all_head_joint"
                if args.arm in ("R0", "R1")
                else "union_plus_one_sensor_round_robin_to_shared"
            ),
            "training_stream_sha256": stream.chain,
            "training_stream_updates": stream.count,
...
def load_resume(path: Path, ddp: DDP, optimizer, scheduler, scaler, args):
    ...
    if (
        cp.get("format") != FORMAT
        or cp.get("arm") != args.arm
        or cp.get("initialization") != "random_from_scratch"
    ):
        raise RuntimeError("Resume checkpoint lineage mismatch")
    if cp.get("source_sha256") != source_fingerprints():
        raise RuntimeError("Training source changed since resumable checkpoint was written")
    previous = cp["args"]
    mutable = {
        "steps",
        "resume_training",
    ...
    for k, v in vars(args).items():
        if k not in mutable and previous.get(k) != v:
            raise RuntimeError(
                f"Resume config mismatch {k}: {previous.get(k)!r} != {v!r}"
            )
    ...
    return (
        int(cp["step"]),
        float(cp["best_val"]),
        cp.get("training_stream_sha256", "0" * 64),
        int(cp.get("training_stream_updates", 0)),
    )
...
def parse_args():
    ...
    for n in (
        ...
        if getattr(args, n) <= 0:
            p.error(f"{n} must be positive")
    if args.global_batch_x % 4 or args.val_global_batch_x % 4:
        p.error("global x batches must be divisible by 4")
...
def main() -> None:
    ...
    try:
        if dist.get_world_size() != 4:
            raise RuntimeError("R012 formal study requires exactly 4 GPUs")
        ...
        base.synchronized_check(
            fresh_ok, f"Output directory nonempty: {out}", device
        )
        if dist.get_rank() == 0:
            out.mkdir(parents=True, exist_ok=True)
        dist.barrier()

        # Common dataset split and model seed. R1 and R2 are therefore exactly
        # identical at initialization. Sampling RNG is reset after construction.
        base.seed_everything(args.seed)
        api = base.load_repo_api(Path(args.repo))
        dataset = api["VisibilityQ0Dataset"](
            path=args.data, val_count=args.val_count, seed=args.seed
        )
        oracle = api["PinocchioFOVOracle"](
            urdf_path=args.urdf,
            joint_names=api["DEFAULT_JOINT_NAMES"],
            sensor_frames=api["DEFAULT_SENSOR_FRAMES"],
        ...
        weights = obj.LossWeights(
            **{k: getattr(args, "weight_" + k) for k in asdict(obj.LossWeights())}
        )
        weights.validate()
        ...
        scaler = torch.cuda.amp.GradScaler(
            enabled=args.amp == "fp16", init_scale=65536.0
        )
        metadata = {
            ...
            "data": args.data,
            "data_bytes": Path(args.data).stat().st_size,
        ...
        if args.resume_training:
            start, best, chain, chain_count = load_resume(
                Path(args.resume_training),
                ddp,
                optimizer,
                scheduler,
                scaler,
            ...
            # Architecture-independent stream control: model construction may use
            # different counts of random numbers, so reset after construction.
            base.seed_everything(args.stream_seed)

        stream = StreamChain(api, chain, chain_count)
        stream.install()

        if dist.get_rank() == 0:
            (out / "train_args.json").write_text(
                json.dumps(
                    {
                        **vars(args),
        ...
        log(
            f"[r012] arm={args.arm} scratch=yes params={model.parameter_count():,} "
            f"world=4 stream_seed={args.stream_seed}"
        )
        log("architecture:", model.architecture())
        log("loss:", asdict(weights))
        ...
        for step in range(start + 1, args.steps + 1):
            ...
            batches, counts = base.prepare_batch(
                api, dataset, oracle, args, device, "train"
            )
            if args.arm == "R2":
                train_stats = run_r2_batch(
                    ddp, batches, counts, weights, args, optimizer, scaler, step
                )
            else:
                train_stats = base.run_prepared_batch(
                    ddp, batches, counts, weights, args, optimizer, scaler
                )
            del batches
            ...
            train_stats.update(
                elapsed_sec=elapsed,
                max_peak_allocated_gib=float(peak),
                training_stream_sha256=stream.chain,
            )

            if step == 1 or step % args.log_every == 0 or step == args.steps:
                h = train_stats["heads"]
                sel = train_stats.get("selected_shared_sensor")
                log(
                    f"[train] {args.arm} {step}/{args.steps} "
                    f"loss={train_stats['loss']:.6f} "
                    f"Ucos={h['union']['grad_cosine']:.4f} "
                    f"S0cos={h['s0']['grad_cosine']:.4f} "
                    f"S6cos={h['s6']['grad_cosine']:.4f} "
                    f"sel={sel} sec={elapsed:.2f} stream={stream.chain[:12]}"
                )
                if dist.get_rank() == 0:
                    with (out / "metrics.jsonl").open("a") as f:
                        f.write(
                            json.dumps(
                                {"split": "train", "step": step, **train_stats},
                                allow_nan=False,
                            )
                            + "\n"
                        )
            ...
            if step == 1 or step % args.val_every == 0 or step == args.steps:
                ...
                vb, vc = base.prepare_batch(
                    api, dataset, oracle, args, device, "val"
                )
                last_val = base.run_prepared_batch(
                    ddp, vb, vc, weights, args
                )
                del vb
                val_elapsed = time.perf_counter() - vt
                best = min(best, last_val["loss"])
                if dist.get_rank() == 0:
                    with (out / "metrics.jsonl").open("a") as f:
                        f.write(
                            json.dumps(
                ...
                log(
                    f"[val] {args.arm} step={step} loss={last_val['loss']:.6f} "
                    f"Ucos={last_val['heads']['union']['grad_cosine']:.4f} "
                    f"S0cos={last_val['heads']['s0']['grad_cosine']:.4f} "
                    f"S6cos={last_val['heads']['s6']['grad_cosine']:.4f}"
                )
            ...
            if step % args.save_every == 0 and step < args.steps:
                save_checkpoint(
                    ...
                    step,
                    best,
                    {"train": train_stats, "val": last_val},
                    stream,
                    metadata,
                )
                if dist.get_rank() == 0:
        ...
        dist.barrier()
        if dist.get_rank() == 0:
            final_sha = sha256_file(out / "final.pt")
            run = json.loads((out / "run.json").read_text())
            run.update(
                status="COMPLETE",
                successful_updates=args.steps,
                training_stream_sha256=stream.chain,
                training_stream_updates=stream.count,
                final_sha256=final_sha,
                best_val=best,
                source_sha256=source_fingerprints(),
                code_sha=metadata["repo_commit"],
            )
            (out / "run.json").write_text(json.dumps(run, indent=2) + "\n")
            print(
                f"[done] r012_arm_complete arm={args.arm} updates={args.steps} "
                f"stream={stream.chain} sha={final_sha}",
                flush=True,
            )
    finally:
