...
echo "[C5.8] COMMIT GATE: exact executable prefix+brake+hold -> final GCDF -> exact VBC -> single commit"
echo "[C5.8] GCDF transport: one GPU client, independent local/final channels, final priority"
echo "[C5.8] EXECUTION: committed trajectory published once; tracker owns execution to completion"
echo "[C5.8] EXECUTION VBC: elapsed suffixes use a separate audit-only topic; tracker is never reset"
echo "[C5.8] PROBE success: tracker complete=1 with matching committed header.stamp token"
