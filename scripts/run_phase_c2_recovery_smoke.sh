...
OUT="${REPO}/outputs/phase_c2_recovery_smoke"
LOG="${REPO}/logs/phase_c2_recovery_smoke"
...
setsid roslaunch egocentric_arm_planner phaseB2_vbc_waypoint_controlled.launch \
  ...
  vbc_min_margin_s:=0.30 trial_label:="${CASE_ID}_recovery_smoke" \
...
echo "[RUN] ${CASE_ID}: initial gate released; observing recovery for ${RUN_SECONDS}s"
sleep "${RUN_SECONDS}"
...
if [ "${RECOVERY_COUNT}" -lt 1 ]; then
  echo "[FAIL] recovery was never entered"
  exit 2
fi
...
echo "[PASS] ${CASE_ID}: intervention -> recovery -> recovery hold -> task replan observed"
echo "[RESULT] ${OUT}"
