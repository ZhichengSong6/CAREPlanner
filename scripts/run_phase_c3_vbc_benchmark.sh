...
OUT="${OUT:-${REPO}/outputs/phase_c3_vbc_benchmark}"
LOG="${LOG:-${REPO}/logs/phase_c3_vbc_benchmark}"
ARCHIVE="${ARCHIVE:-${REPO}/phase_c3_vbc_benchmark_12x3.zip}"
...
POST_RELEASE_SECONDS="${POST_RELEASE_SECONDS:-6.0}"
...
python3 - "${OUT}/benchmark_metadata.json" "${CASE_FILE}" "${GIT_COMMIT}" "${GIT_BRANCH}" \
  "${BASELINE_WEIGHT}" "${CARE_WEIGHT}" "${SAFETY_MARGIN}" "${PRED_ERROR_INF}" "${PRED_STREAK}" "${PRED_MIN_IMPROVEMENT_INF}" <<'PYMETA'
import json,sys,time
(out,case_file,commit,branch,w0,w1,margin,pred_err,pred_streak,pred_improvement)=sys.argv[1:]
p={
  ...
  'predictive_recovery':{
    'error_threshold_inf':float(pred_err),
    'consecutive_misses_required':int(pred_streak),
    'min_improvement_inf_diagnostic':float(pred_improvement),
    'require_insufficient_progress':False,
  },
  'gazebo_gui':False,
  'rviz':False,
  'paired_design':'same frozen case, fresh headless Gazebo each run; baseline vs deadline recovery vs predictive recovery'
}
...
p=sys.argv[1]
d=json.load(open(p))
ids=d.get('selected_case_ids',[]); cases=d.get('cases',[])
...
if timeout 2 rosnode list >/dev/null 2>&1; then
  echo "[ERROR] A ROS master is already running. Close other ROS/Gazebo launches first."
  exit 1
fi
...
mode_fields() {
  case "$1" in
    baseline) echo "${BASELINE_WEIGHT} false false" ;;
    deadline_recovery) echo "${CARE_WEIGHT} true false" ;;
    predictive_recovery) echo "${CARE_WEIGHT} true true" ;;
    *) return 2 ;;
  esac
}
...
cleanup_owned_ros_fallback() {
  ...
  sleep 1
  for name in gzclient gzserver rviz rosmaster roscore roslaunch; do pkill -KILL -x "${name}" 2>/dev/null || true; done
}
...
cleanup_run() {
  ...
  echo "[WARN] ROS master still reachable; applying owned-process fallback"
  cleanup_owned_ros_fallback
  for _ in $(seq 1 30); do
    if ! timeout 1 rosnode list >/dev/null 2>&1; then return 0; fi
    sleep 0.1
...
wait_joint_state() {
  ...
  for _ in $(seq 1 120); do
    if timeout 2 rostopic echo -n 1 /care_arm/joint_states > "${out}" 2>/dev/null; then return 0; fi
    sleep 0.25
  done
  return 1
}
...
case_file,cid,js_file,out_file,tol=sys.argv[1:]; tol=float(tol)
...
q=[float(pos[idx[n]]) for n in joints]; q0=[float(x) for x in c['initial_q']]
err=max(abs(a-b) for a,b in zip(q,q0))
p={'case_id':cid,'expected_initial_q':q0,'measured_initial_q':q,'error_inf_rad':err,'tolerance_rad':tol,'passed':err<=tol}
json.dump(p,open(out_file,'w'),indent=2)
...
d=json.load(open(case_file)); c=next(x for x in d['cases'] if x['case_id']==cid)
text=open(summary_file).read(); m=re.search(r'frozen_target=\[([^\]]+)\].*?frozen_sweep_time_s=([+\-0-9.eE]+)',text,re.S)
...
json.dump({'case_id':cid,'expected_target_xyz':expected,'actual_target_xyz':actual,'target_error_inf_m':target_err,'target_tolerance_m':target_tol,'expected_sweep_time_s':expected_sweep,'actual_sweep_time_s':actual_sweep,'sweep_error_s':sweep_err,'sweep_tolerance_s':sweep_tol,'passed':passed},open(out_file,'w'),indent=2)
...
for path in paths:
    ...
    score=errs['target_error_inf_m']/max(target_tol,1e-12)+errs['sweep_error_s']/max(sweep_tol,1e-12)+errs['q_nom_error_inf_rad']/max(q_tol,1e-12)+errs['q_vis_error_inf_rad']/max(q_tol,1e-12)
    candidates.append((score,path,errs))
if not candidates: raise SystemExit('no readable runtime projector trace')
score,path,errs=min(candidates,key=lambda x:x[0])
...
json.dump({'case_id':cid,'trace_file':path,'num_trace_candidates':len(candidates),'match_score':score,**errs,'passed':passed,'tolerances':{'target_m':target_tol,'sweep_s':sweep_tol,'q_nom_rad':q_tol,'q_vis_rad':q_tol}},open(out_file,'w'),indent=2)
print('[CHECK] %s projector matched %s target_err=%.3g sweep_err=%.3g q_nom_err=%.3g q_vis_err=%.3g' % (cid,os.path.basename(path),errs['target_error_inf_m'],errs['sweep_error_s'],errs['q_nom_error_inf_rad'],errs['q_vis_error_inf_rad']))
raise SystemExit(0 if passed else 2)
PYPROJ
}
...
wait_gate_release() {
  ...
  for _ in $(seq 1 300); do
    ...
    if echo "${tmp}" | grep -q "released=1"; then printf '%s\n' "${tmp}" > "${out}"; return 0; fi
    sleep 0.1
  done
  return 1
}
...
write_run_status() {
  local run_dir="$1" cid="$2" difficulty="$3" mode="$4" weight="$5" recovery="$6" predictive="$7" status="$8" message="$9"
  python3 - "${run_dir}" "${cid}" "${difficulty}" "${mode}" "${weight}" "${recovery}" "${predictive}" "${status}" "${message}" <<'PY'
import json,sys,time
run_dir,cid,difficulty,mode,weight,recovery,predictive,status,message=sys.argv[1:]
p={'case_id':cid,'difficulty_bin':difficulty,'mode':mode,'waypoint_weight':float(weight),'recovery_enabled':recovery.lower()=='true','predictive_recovery_enabled':predictive.lower()=='true','status':status,'message':message,'wall_time':time.strftime('%Y-%m-%dT%H:%M:%S%z')}
json.dump(p,open(run_dir+'/run_status.json','w'),indent=2)
PY
}

FAILED=0; TOTAL=0; EXPECTED=$((12 * ${#MODES[@]}))
for CASE_ID in "${CASE_IDS[@]}"; do
  IFS=$'\t' read -r CID DIFFICULTY GX GY GZ GQX GQY GQZ GQW TX TY TZ SWEEP <<< "$(case_fields "${CASE_ID}")"

  for MODE in "${MODES[@]}"; do
    TOTAL=$((TOTAL+1))
    read -r WEIGHT RECOVERY_ENABLED PREDICTIVE_ENABLED <<< "$(mode_fields "${MODE}")"
    ...
    echo
    echo "================================================================"
    echo "[RUN ${TOTAL}/${EXPECTED}] ${CID} difficulty=${DIFFICULTY} mode=${MODE} weight=${WEIGHT} recovery=${RECOVERY_ENABLED} predictive=${PREDICTIVE_ENABLED}"
    echo "================================================================"

    STATUS="ok"; MESSAGE="completed"
    if timeout 2 rosnode list >/dev/null 2>&1; then STATUS="preexisting_ros_master"; MESSAGE="ROS master was still running before run"; fi

    if [ "${STATUS}" = "ok" ]; then
      setsid roslaunch arm_description gazebo_velocity_control.launch gazebo_gui:=false use_rviz:=false > "${RUN_LOG}/gazebo.log" 2>&1 &
      GAZEBO_PID=$!
      if ! wait_joint_state "${RUN_OUT}/initial_joint_state.yaml"; then STATUS="gazebo_joint_state_timeout"; MESSAGE="no /care_arm/joint_states after Gazebo startup"; fi
    fi

    if [ "${STATUS}" = "ok" ]; then
      if ! validate_initial_q "${RUN_OUT}/initial_joint_state.yaml" "${CID}" "${RUN_OUT}/initial_q_validation.json"; then STATUS="initial_q_mismatch"; MESSAGE="Gazebo initial q does not match frozen case"; fi
    fi
    ...
    if [ "${STATUS}" = "ok" ]; then
      ...
      if [ "${READY}" != "1" ]; then STATUS="waypoint_generator_not_ready"; MESSAGE="VisCDF waypoint generator failed to become ready"; fi
    fi
    ...
    if [ "${STATUS}" = "ok" ]; then
      setsid roslaunch egocentric_arm_planner phaseB2_vbc_waypoint_controlled.launch \
        ...
        vbc_min_margin_s:="${SAFETY_MARGIN}" trial_label:="${RUN_NAME}" \
        log_output_root:="${RUN_OUT}" use_trajectory_visualizer:=false \
      ...
      if ! wait_frozen_summary "${RUN_OUT}/controlled_trial_summary.yaml"; then
        ...
        STATUS="frozen_case_mismatch"; MESSAGE="runtime target/sweep differs from frozen case"
        ...
        echo "[RUN] gate released; recording ${POST_RELEASE_SECONDS}s"
        sleep "${POST_RELEASE_SECONDS}"
      fi
    fi

    if ! cleanup_run; then
      if [ "${STATUS}" = "ok" ]; then STATUS="cleanup_failed"; MESSAGE="ROS processes survived run cleanup"; fi
    fi

    if [ "${STATUS}" = "ok" ]; then
      if ! validate_projector "${RUN_OUT}" "${CID}" "${RUN_OUT}/runtime_projector_validation.json"; then
        STATUS="runtime_projector_mismatch"; MESSAGE="no projector trace matches frozen case within tolerance"
        ...
        STATUS="missing_mpc_summary"; MESSAGE="waypoint MPC logger summary missing"
      elif ! compgen -G "${RUN_OUT}/*_weight_*/per_cycle.csv" >/dev/null; then
        STATUS="missing_per_cycle_csv"; MESSAGE="per-cycle MPC/predictive logger CSV missing"
      fi
    fi

    write_run_status "${RUN_OUT}" "${CID}" "${DIFFICULTY}" "${MODE}" "${WEIGHT}" "${RECOVERY_ENABLED}" "${PREDICTIVE_ENABLED}" "${STATUS}" "${MESSAGE}"
    if [ "${STATUS}" != "ok" ]; then
      FAILED=$((FAILED+1)); echo "[FAIL] ${CID}/${MODE}: ${STATUS} - ${MESSAGE}"
    else
      echo "[DONE] ${CID}/${MODE}"
    fi
    sleep 1
...
python3 scripts/summarize_phase_c3_vbc_benchmark.py --case-file "${CASE_FILE}" --output-root "${OUT}"

python3 - "${OUT}" "${LOG}" "${ARCHIVE}" <<'PYPACK'
import os,sys,zipfile
out,log,archive=sys.argv[1:]
with zipfile.ZipFile(archive,'w',compression=zipfile.ZIP_DEFLATED) as z:
    for root,label in ((out,'outputs/phase_c3_vbc_benchmark'),(log,'logs/phase_c3_vbc_benchmark')):
...
echo "[SUMMARY] runs: ${OUT}/benchmark_runs.csv"
echo "[SUMMARY] paired: ${OUT}/paired_summary.csv"
echo "[SUMMARY] representatives: ${OUT}/representative_case_candidates.json"
