...
OUT_ROOT="${OUT_ROOT:-${REPO}/outputs/phase_c4_1_frozen12_benchmark}"
LOG_ROOT="${LOG_ROOT:-${REPO}/logs/phase_c4_1_frozen12_benchmark}"
ARCHIVE="${ARCHIVE:-${REPO}/phase_c4_1_frozen12_benchmark.zip}"
...
python3 - "${OUT_ROOT}/benchmark_metadata.json" "${CASE_FILE}" "${GIT_COMMIT}" "${GIT_BRANCH}" "${RUN_SECONDS}" "${CARE_WEIGHT}" "${SAFETY_MARGIN}" <<'PY'
import json,sys,time
out,case_file,commit,branch,run_s,weight,margin=sys.argv[1:]
p={
  ...
  'git_branch':branch,
  'num_cases':12,
  'mode':'deadline_recovery_plus_c4_1_diagnostic',
  'c3_predictive_recovery_enabled':False,
  'c4_1_predicted_vbc_auditor_enabled':True,
  'c4_1_controls_robot':False,
  'post_release_observation_s':float(run_s),
  'waypoint_weight':float(weight),
  'safety_margin_s':float(margin),
  'ground_truth':'whether the same run actually enters Deadline Recovery',
}
...
for cid in "${CASE_IDS[@]}"; do
  ...
  if CASE_ID="${cid}" \
     ...
     bash scripts/run_phase_c4_1_audit_smoke.sh; then
    RC=0
    STATUS="ok"
    MESSAGE="completed"
  else
...
python3 scripts/summarize_phase_c4_1_frozen12.py \
  --case-file "${CASE_FILE}" \
...
echo "[DONE] Phase C4.1 frozen-12 diagnostic benchmark complete"
echo "[RESULT] ${OUT_ROOT}/benchmark_summary.json"
echo "[TABLE]  ${OUT_ROOT}/case_results.csv"
echo "[STREAK] ${OUT_ROOT}/classification_by_streak.csv"
echo "[ZIP]    ${ARCHIVE}"
