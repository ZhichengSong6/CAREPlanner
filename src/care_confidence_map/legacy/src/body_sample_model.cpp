...
// Optional compatibility entry; old C++ API and its symbols remain available.
extern "C" bool care_load_body_samples_v1(care_confidence_map::BodySampleModel* model,
    const std::string& file, std::string* error) {
  return model && model->loadFromYaml(file, error);
}
