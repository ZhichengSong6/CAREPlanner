...
def main():
    ...
    parser.add_argument("--p-num-shards", type=int, default=1,
                        help="Split selected p_indices into this many disjoint shards. Default 1 preserves single-GPU behavior.")
    parser.add_argument("--p-shard-id", type=int, default=0,
                        help="This process handles selected_p_indices[p_shard_id::p_num_shards].")
    ...
    if args.p_count > 0:
        selected_p_indices = p_rng.choice(len(grid_points), size=min(args.p_count, len(grid_points)), replace=False)
    else:
        selected_p_indices = np.arange(len(grid_points), dtype=np.int64)

    if args.p_num_shards < 1:
        raise ValueError("--p-num-shards must be >= 1")
    if args.p_shard_id < 0 or args.p_shard_id >= args.p_num_shards:
        raise ValueError("--p-shard-id must satisfy 0 <= p_shard_id < p_num_shards")

    p_indices = selected_p_indices[args.p_shard_id::args.p_num_shards]
    if len(p_indices) == 0:
        raise RuntimeError(
            f"Empty p shard: selected_p_count={len(selected_p_indices)} "
            f"p_shard_id={args.p_shard_id} p_num_shards={args.p_num_shards}"
        )
    ...
    print(f"p_shard:         {args.p_shard_id}/{args.p_num_shards}")
    ...
    save_data = {
        ...
        "p_num_shards": np.asarray(args.p_num_shards, dtype=np.int32),
        "p_shard_id": np.asarray(args.p_shard_id, dtype=np.int32),
        "selected_p_count_before_shard": np.asarray(len(selected_p_indices), dtype=np.int32),
    }
    if args.extract_per_sensor:
        save_data.update({
