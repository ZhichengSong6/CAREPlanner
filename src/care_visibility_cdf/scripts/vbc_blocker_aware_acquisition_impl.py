...
class BlockerAwareVisibilityAcquisitionWaypointNode(VisibilityAcquisitionWaypointNode):
    ...
    def _build_certified_frontier_bank(
            self, active_id: int, active, measured: np.ndarray):
        """Generate a small cached bank of alternative q_vis basins.

        The existing obligation q_vis is retained as one candidate. Additional
        candidates are generated by perturbing the measured-q seed along the
        largest learned visibility-gradient joints, then rerunning the same
        learned projector. Only a short local step toward each q_vis is sent to
        the planner; final GCDF + exact VBC remain the commit authorities.
        """
        if not self.certified_frontier_search_enabled:
            return None

        points = np.asarray(
            active.get("points", []), dtype=np.float64).reshape(-1, 3)
        base_q_vis = np.asarray(
            active.get("q_vis", []), dtype=np.float64).reshape(-1)
        if (points.shape[0] == 0 or not np.all(np.isfinite(points)) or
                base_q_vis.shape != (7,) or
                not np.all(np.isfinite(base_q_vis))):
            return None

        quantized_q = tuple(
            int(round(float(v) / self.certified_frontier_recompute_q_inf))
            for v in measured)
        geometry_key = tuple(active.get("keys", ()))
        bank_key = (int(active_id), geometry_key, quantized_q)
        with self._certified_frontier_lock:
            if (self._certified_frontier_bank_key == bank_key and
                    self._certified_frontier_bank):
                return list(self._certified_frontier_bank)
        ...
        sweep_s = float(active.get(
            "discovered_sweep_time_s",
            max(self.safety_margin_s, 0.30)))
        if not math.isfinite(sweep_s) or sweep_s < 0.0:
            sweep_s = max(self.safety_margin_s, 0.30)
        ...
        try:
            ...
            grad = grad_tensor[0].detach().cpu().numpy().astype(np.float64)
        except Exception as exc:
            self._frontier_error_count += 1
            rospy.logerr_throttle(
                1.0,
                "[certified_frontier] seed gradient failed: %s", exc)
            return None
        if grad.shape != (7,) or not np.all(np.isfinite(grad)):
            return None

        current_values = self._per_point_values(x, q_measured)
        current_f_min = (
        ...
        for joint in order:
            sign = 1.0 if grad[joint] >= 0.0 else -1.0
            for signed in (sign, -sign):
                seed_specs.append((
                    "j{}_{:+.3f}".format(
                        int(joint) + 1,
                        signed * self.certified_frontier_seed_perturbation),
                    (int(joint), float(
                        signed * self.certified_frontier_seed_perturbation))))
                if len(seed_specs) >= self.certified_frontier_seed_count:
                    break
            if len(seed_specs) >= self.certified_frontier_seed_count:
                break

        raw_candidates = []
        for seed_label, perturb in seed_specs:
            q_vis = None
            generation_ms = 0.0
            if perturb is None:
                q_vis = base_q_vis.copy()
            else:
                joint, delta = perturb
                seed = measured.copy()
                seed[joint] += delta
                seed_tensor = torch.tensor(
                    seed.reshape(1, 7),
                    device=self.device, dtype=torch.float32)
                seed_tensor, _ = self._clamp(seed_tensor)
                seed = seed_tensor[0].detach().cpu().numpy().astype(
                    np.float64)

                self._seed_override = seed.copy()
                started = time.perf_counter()
                try:
                    result = self._generate_active_set_waypoint(
                    ...
                    rospy.logwarn_throttle(
                        1.0,
                        "[certified_frontier] alternative q_vis generation "
                        "failed seed=%s: %s", seed_label, exc)
                    q_vis = None
                finally:
                    generation_ms = 1000.0 * (
                        time.perf_counter() - started)
                    projector_ms += generation_ms
                    self._seed_override = None

            if (q_vis is None or q_vis.shape != (7,) or
                    not np.all(np.isfinite(q_vis))):
                continue

            delta_q = q_vis - measured
            delta_inf = float(np.max(np.abs(delta_q)))
            if delta_inf <= self.frontier_gradient_eps:
                target = q_vis.copy()
            else:
                target = (
                    measured +
                    min(self.frontier_step_inf, delta_inf) *
                    delta_q / delta_inf)
            ...
            gain = target_f_min - current_f_min
            qvis_dist = float(np.max(np.abs(q_vis - measured)))
            score = float(gain - 0.01 * qvis_dist)

            # Keep the existing q_vis as a fail-closed baseline even if its
            # immediate learned gain is weak. Alternative seeds must at least
            # avoid a strongly negative local regression.
            if (seed_label != "existing_qvis" and
                    (not math.isfinite(gain) or
                     gain < self.certified_frontier_min_gain)):
                continue

            duplicate = any(
                np.max(np.abs(target - item["target"])) <= 1e-3
        ...
        if not raw_candidates:
            delta_q = base_q_vis - measured
            delta_inf = float(np.max(np.abs(delta_q)))
            target = (
                base_q_vis.copy()
                if delta_inf <= self.frontier_gradient_eps
                else measured + min(
                    self.frontier_step_inf, delta_inf) *
                    delta_q / delta_inf)
            raw_candidates = [{
                "seed_label": "existing_qvis_fallback",
                "q_vis": base_q_vis.copy(),
                "target": target.copy(),
                "gain": math.nan,
                "score": -math.inf,
                "target_f_min": math.nan,
                "qvis_dist_inf": delta_inf,
                "projector_ms": 0.0,
            }]

        build_ms = 1000.0 * (time.perf_counter() - build_start)
        with self._certified_frontier_lock:
            self._certified_frontier_bank_key = bank_key
            self._certified_frontier_bank = list(raw_candidates)
            self._certified_frontier_index = 0
            self._certified_frontier_exhausted = False
            self._certified_frontier_last_vbc_stamp_ns = "none"
            self._certified_frontier_bank_build_count += 1
            self._certified_frontier_bank_build_ms = float(build_ms)
            self._certified_frontier_projector_ms = float(projector_ms)
            self._certified_frontier_screen_ms = float(screen_ms)
            self._certified_frontier_last_selected_score = float(
                raw_candidates[0]["score"])
            self._certified_frontier_last_selected_seed = str(
                raw_candidates[0]["seed_label"])

        rospy.logwarn(
            "[certified_frontier] BANK active_O=%d size=%d build=%.3fms "
            "projector=%.3fms screen=%.3fms seeds=%s",
            int(active_id), len(raw_candidates), build_ms,
            projector_ms, screen_ms,
            ",".join(str(v["seed_label"]) for v in raw_candidates))
        return list(raw_candidates)

    def _candidate_vbc_summary_callback(self, msg: String) -> None:
        """Advance alternative sensing targets only on a NEW exact-VBC verdict."""
        super()._candidate_vbc_summary_callback(msg)
        if (msg is None or
                not getattr(self, "certified_frontier_search_enabled", False)):
            return
        ...
        with self._certified_frontier_lock:
            if stamp == self._certified_frontier_last_vbc_stamp_ns:
                return
            self._certified_frontier_last_vbc_stamp_ns = stamp
            if not self._certified_frontier_bank:
                return

            if fields.get("has_violation") == "0":
                self._certified_frontier_safe_candidate_count += 1
                return
            if fields.get("has_violation") != "1":
                return

            if (self._certified_frontier_index + 1 <
                    len(self._certified_frontier_bank)):
                self._certified_frontier_index += 1
                item = self._certified_frontier_bank[
                    self._certified_frontier_index]
                self._certified_frontier_unsafe_switch_count += 1
                self._certified_frontier_last_selected_score = float(
                    item["score"])
                self._certified_frontier_last_selected_seed = str(
                    item["seed_label"])
                ...
                self._frontier_cache = None
                rospy.logwarn(
                    "[certified_frontier] exact VBC UNSAFE stamp=%s -> "
                    "alternative %d/%d seed=%s score=%+.5f",
                    stamp, self._certified_frontier_index + 1,
                    len(self._certified_frontier_bank),
                    item["seed_label"], float(item["score"]))
            elif not self._certified_frontier_exhausted:
                self._certified_frontier_exhausted = True
                self._certified_frontier_exhausted_count += 1
                self._certified_frontier_stuck_count += 1
                self._frontier_cache_key = None
                self._frontier_cache = None
                rospy.logerr(
                    "[certified_frontier] CERTIFIED_FRONTIER_STUCK "
                    "all %d alternatives exact-VBC unsafe",
                    len(self._certified_frontier_bank))
